<template>
	<div class="pelanggan">
		...
		<v-dialog required v-model="dialog" width="500">
			<v-card>
		...
		<!-- ====================================== FORM EDIT ============================================= -->
		<v-dialog required v-model="editDialog" width="500">
			<v-card>
		...
		<v-dialog v-model="dialogKonfirmasi" persistent max-width="350">
			<v-card>
							...
							<v-row class>
								...
								<v-menu offset-y>
									<template v-slot:activator="{ on }">
										<v-btn class="mx-1 mb-2 blue-grey lighten-5 blue-grey--text" v-on="on" small depressed>
											<v-icon left>filter_list</v-icon>
											<span>Dusun</span>
										</v-btn>
									</template>
									<v-list>
										<v-list-item v-for="(item, index) in dusuns" :key="index">
								...
								<!-- ============================== Filter bintang ======================================== -->
								<v-btn
									class="mx-1 mb-2 blue-grey lighten-5 blue-grey--text"
									active-class="primary"
									:class="{primary:filterBintang}"
									small
									depressed
								>
									<!-- <v-icon left>filter_list</v-icon> -->
...
export default {
	...
	data() {
		return {
			...
			formteleponRules: [
				v => !!v || "Harus diisi",
				v => (v && v.length >= 11) || "Nomor telpon tidak benar"
			],
			selectedDusun: "",
			selectedRt: "",
			rtTersedia: [],
			selectedJenis: "",
			jenisTersedia: ["Rumah", "Toko", "Sekolah"],
			disabledbtn: true,
	...
	methods: {
		simpanpelangganbaru() {
			...
			let tanggal_masuk = new Date(Date.now());
		...
		//   filterdusun(index){
		//     this.filtered = this.data_pelanggan.filter((da)=>da['dusun'].toLowerCase().includes(this.dusuns[index].toLowerCase()))
		//     this.filter = true
		//   },
		//   filterbintang(){
		//     if(this.filter){
		//       if(this.sort){
		//         var sort = []
		//         this.data_pelanggan.forEach(e=>{
		//           sort.push(e)
		//         })
		//         this.filtered = sort.sort((a,b) => (a['name'] > b['name']) ? 1 : ((b['name'] > a['name']) ? -1 : 0))
		//         this.filter = false
		//       }else{
		//         this.filtered = this.data_pelanggan
		//         this.filter = false
		//       }
		//     }else if(!this.filter){
		//       this.filtered = this.filtered.filter((da)=>da['bintang']==true)
		//       this.filter = true
		//     }
		...
		//         tes.push(e)
		//       })
		//       this.filtered = tes.sort((a,b) => (a['name'] > b['name']) ? 1 : ((b['name'] > a['name']) ? -1 : 0))
		//       this.sort = true
		//     }
		//  },
		setTanggal() {
		...
		resetform() {
			this.dialog = false;
			this.loader = false;
			this.$refs.form.resetValidation(), (this.selectedDusun = "");
			this.selectedRt = "";
			this.formnama = "";
			this.formnik = "";
			this.formtelepon = "";
